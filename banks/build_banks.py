...
def add_30257():
    """Exam-paper questions, L6 workbook problems, gap-filling Hard questions, units and ledger for 30257 (see c30257.py)."""
    import c30257 as c
    for q in BANKS["30257"]["questions"]:
        q["units"] = c.EXISTING_UNITS[q["id"]]
        q["slide_groups"] = c.EXISTING_GROUPS[q["id"]]
    BANKS["30257"]["questions"] += c.REAL + c.HARD
    fp = COURSES["30257"]["fingerprint"]
    fp["by_shape"]["concept MCQ"] = c.MCQ_SHAPE["exam"]
    fp["hard_target"]["by_shape"]["concept MCQ"] = c.MCQ_SHAPE["target"]
    fp["sources"].append("AY 24-25 / AY 26-27 mocks, MCQs (1-3 steps each; median 1,5) - scored 2026-09-24 for the concept MCQ shape")
    for d, secs in c.SECTIONS.items():
        COURSES["30257"]["decks"][d]["sections"] = secs
        COURSES["30257"]["decks"][d]["units"] = c.UNITS[d]
    COURSES["30257"]["source_ledger"] = c.LEDGER
    BANKS["30257"]["coverage"] = {"course": "30257", "sections": c.SECTIONS, "units": c.UNITS, "existing_units": c.EXISTING_UNITS, "ledger": c.LEDGER}


def add_30024():
    """Past-paper and handout questions, gap-filling Hard questions, three exam shapes, units and ledger for 30024 (see c30024.py)."""
    import c30024 as c
    for q in BANKS["30024"]["questions"]:
        q["units"] = c.EXISTING_UNITS[q["id"]]
        q["slide_groups"] = c.EXISTING_GROUPS[q["id"]]
    BANKS["30024"]["questions"] += c.REAL + c.HARD
    co = COURSES["30024"]
    fp = co["fingerprint"]
    fp["by_shape"]["concept MCQ"] = c.MCQ_SHAPE["exam"]
    fp["hard_target"]["by_shape"]["concept MCQ"] = c.MCQ_SHAPE["target"]
    fp["by_shape"]["qualitative essay part"] = c.QUAL_SHAPE["exam"]
    fp["slide_baseline"]["by_shape"]["qualitative essay part"] = c.QUAL_SHAPE["slide"]
    fp["hard_target"]["by_shape"]["qualitative essay part"] = c.QUAL_SHAPE["target"]
    fp["sources"] += ["2023 mock Part A, 15 MCQs (1-4 steps; median 3) - scored 2026-09-24 for the concept MCQ shape",
                      "2023 mock Part B open essays and the 2017 short essays (2-4 points; median 3) - scored 2026-09-24 for the qualitative essay shape"]
    co["format_profile"][0] = {"item": "Final written exam (attending, Part A)",
                               "value": "A mix of open essay questions, with and without calculations, and multiple choices (syllabus 2026-27); the 2023 mock had 15 MCQs and a 10-point quantitative essay in 1 hour. 50% of the grade",
                               "label": "COURSE-AUTHORITATIVE (syllabus 2026-27, section 6) for the mix; SUPPORTING (2023 mock) for the counts",
                               "settle_with": "the exam direction sheet (late November)"}
    for d, secs in c.SECTIONS.items():
        co["decks"][d]["sections"] = secs
        co["decks"][d]["units"] = c.UNITS[d]
    co["decks"]["S1"]["reading"] = [c.F_S1T]
    co["source_ledger"] = c.LEDGER
    BANKS["30024"]["coverage"] = {"course": "30024", "sections": c.SECTIONS, "units": c.UNITS, "existing_units": c.EXISTING_UNITS, "ledger": c.LEDGER}


def add_30285():
    """Past-exam and practice questions, gap-filling Hard questions, units and ledger for 30285 (see c30285.py)."""
    import c30285 as c
    for q in BANKS["30285"]["questions"]:
        q["units"] = c.EXISTING_UNITS[q["id"]]
        q["slide_groups"] = c.EXISTING_GROUPS[q["id"]]
    BANKS["30285"]["questions"] += c.REAL + c.HARD
    co = COURSES["30285"]
    co["format_profile"][0]["value"] += "; exam first page: 15 MCQs in 35 minutes"
    co["format_profile"][0]["label"] += "; Exam_First_Page.docx"
    co["fingerprint"]["sources"] += ["Past finals (20 Dec 2021 and another sitting) and Practice #1/#2: in-scope items are 1-step concept MCQs and 3-step variance-of-a-combination items - read 2026-09-24, consistent with the median of 3"]
    for d, secs in c.SECTIONS.items():
        co["decks"][d]["sections"] = secs
        co["decks"][d]["units"] = c.UNITS[d]
    co["source_ledger"] = c.LEDGER
    BANKS["30285"]["coverage"] = {"course": "30285", "sections": c.SECTIONS, "units": c.UNITS, "existing_units": c.EXISTING_UNITS, "ledger": c.LEDGER}


# Own-deck slide for questions whose only citations point at another deck or a handout (pages checked in the PDFs 2026-09-24).
OWN_DECK_CITES = {
    **{q: ("S1", 25, "WHAT DO YOU (NOT) KNOW ABOUT ACCOUNTING?") for q in ["30024-S1-10", "30024-S1-11", "30024-S1-12", "30024-S1-13", "30024-S1-14", "30024-S1-15", "30024-S1-16", "30024-S1-04"]},
    "30178-SVB-08": ("SVB", 3, "Readings to prepare the discussion (all available in B-Board)"),
    "30178-SVB-09": ("SVB", 3, "Readings to prepare the discussion (all available in B-Board)"),
    "30257-L3-11": ("L3", 18, "The most commonly applied formats relax the no-growth pattern, breaking down the growing forecast period in two stages"),
    "30257-L4-17": ("L4", 27, "Based on the tax code of the Country, a rate which allows the deductibility of interest expenses"),
    "30257-L6-10": ("L6", 2, "Omega Inc. closed FY 2019 with the following balance sheet. Reorganize it according to its sources and uses."),
    "30257-L6-11": ("L6", 3, "How much is the company’s FCFO for valuation purposes in 2019?"),
    "30257-L6-12": ("L6", 4, "Full House LLC, a real estate company, discloses its FY 2019 annual report"),
    "30257-L6-13": ("L6", 5, "Estimate Champagne SA’s WACC considering that its 2019 book value of equity is €300 million"),
    "30257-L6-14": ("L6", 7, "A renewable energy group, Solar Beam SA, is developing a solar field"),
}


def map_slides():
    """Every bank question gets numeric slides: title citations are resolved to PDF page numbers through slide_index.json
    (built by index_slides.py from the decks: each quote was found on that page), and q.slides lists the pages of the
    question's own deck that its citations point to."""
    idx = json.loads((OUT / "slide_index.json").read_text())
    for code, b in BANKS.items():
        for q in b["questions"]:
            if q["id"] in OWN_DECK_CITES and not any(c["deck"] == q["deck"] for c in q["citations"]):
                q["citations"].append(cite(*OWN_DECK_CITES[q["id"]]))
            for c in q["citations"]:
                if not isinstance(c["slide"], int):
                    key = f'{q["course"]}|{c["deck"]}|{c["slide"]}|{c["quote"]}'
                    assert key in idx["index"], "citation not in slide_index.json (run index_slides.py): " + key
                    c["title"], c["slide"] = c["slide"], idx["index"][key]
            if not q.get("slides"):
                q["slides"] = sorted({c["slide"] for c in q["citations"] if c["deck"] == q["deck"]})
            assert q["slides"], q["id"] + " has no slide in its own deck"
        co = COURSES.get(code)
        if co:
            for d, deck in co["decks"].items():
                n = idx["page_counts"].get(f"{code}|{d}")
                if n and not deck.get("slide_count"):
                    deck["slide_count"] = n
