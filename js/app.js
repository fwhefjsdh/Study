...
async function blobUrl(id){if(blobUrls.has(id))return blobUrls.get(id);const b=await AGStore.getBlob(id);if(!b)return null;const u=URL.createObjectURL(b);blobUrls.set(id,u);return u;}
...
function route(){
  ...
  if(S.A&&!S.A.submitted&&S.A.strokes.length&&!(p0==="q"&&parts[1]===S.A.qid)){
    if(!confirm("Leave this question? Your selections and reasoning are kept as a draft, but the sketch is not.")){history.replaceState(null,"","#/q/"+encodeURIComponent(S.A.qid));return;}
  }
...
function viewQueue(){
  ...
  o+="</section>";
  if(!Object.keys(S.questions).length)return o+'<div class="panel empty"><p><strong>No questions yet.</strong></p><p>Go to <a href="#/settings">Settings</a> to load the bundled 30178 bank, or <a href="#/import">import a bank file</a>.</p></div>';
...
function viewSettings(){
  ...
  o+='<div class="panel stack"><h3>Your data</h3><p class="small muted">Stored in this browser only ('+(AGStore.mode==="indexeddb"?"IndexedDB":"memory: not saved")+"). "+docs+' records. Download a backup now and then, and before clearing browser data or switching device.</p>'+
    ...
    '<div class="row"><button class="btn sm" data-act="mergeSeed">Add the bundled 30178 bank</button><span class="small muted">Adds any bundled questions you don\'t have. Never overwrites your progress.</span></div>'+
    '<details><summary><span class="label" style="color:var(--bad)">Erase everything</span></summary><div class="row" style="margin-top:8px"><input type="text" id="eraseConfirm" placeholder="Type ERASE" autocomplete="off"><button class="btn sm danger solid" data-act="eraseAll">Erase this browser\'s data</button></div></details></div>';
...
function checkImport(text){
  ...
  const inCourses=asMap(data.courses);if(data.course&&typeof data.course==="object"&&data.course.code)inCourses[data.course.code]=data.course;
...
function renderAnswer(){
  ...
  o+='<div class="stack"><label for="reasoning" class="label">Your reasoning</label><textarea id="reasoning" placeholder="Why you chose each answer: steps, classifications, assumptions.'+(S.settings.apiKey?" Claude marks this.":"")+'"'+(A.submitted?" readonly":"")+"></textarea></div>";
...
async function submitAnswer(){
  ...
  if(S.settings.apiKey&&(A.typed.trim()||written)){A.markStatus="running";renderReveal();renderActionBar();
...
async function finalize(verdict,how,easy){
  ...
  try{const b=await A.sketchPromise;if(b){const sid="sk-"+A.attemptId;await AGStore.putBlob(sid,b);att.sketch_asset=sid;A.sketch=sid;}}catch(e){A.sketchNote="The sketch could not be saved in this browser.";}
...
function renderExport(){
  ...
  const fmt=[["md","Markdown (.md)","One readable file: questions, your answers, feedback, key"],["print","Print or save as PDF","Formatted pages, with your sketches"],["zip","Folder (.zip)","One Markdown file per question, plus sketches and an index"],["csv","Spreadsheet (.csv)","One row per attempt"],["json","Data (.json)","Re-importable into Answer Grid"]];
  DLG.innerHTML='<form method="dialog" class="stack" id="exForm"><div class="row between"><div><div class="label">Export</div><h2>'+h(ex.title)+'</h2></div><button class="btn sm ghost" value="cancel" aria-label="Close">✕</button></div>'+
    ...
    '<div class="row between"><span class="small muted" id="exMsg"></span><span class="row">'+(ex.format==="md"?'<button class="btn" type="button" data-act="exCopy"'+(items.length?"":" disabled")+">Copy</button>":"")+'<button class="btn primary" type="button" data-act="exGo"'+(items.length?"":" disabled")+">"+(ex.format==="print"?"Open print view":"Download")+"</button></span></div></form>";
}
...
async function runExport(copyOnly){
  ...
  if(ex.format==="md"){const md=E.exportMarkdown(ex.title,items,opts);if(copyOnly){copyText(md);return;}download(base+".md",md,"text/markdown;charset=utf-8");}
  else if(ex.format==="csv")download(base+".csv",E.exportCsv(items),"text/csv;charset=utf-8");
    ...
    download(base+".json",JSON.stringify({app:"answer-grid",format:1,exported_at:new Date().toISOString(),title:ex.title,courses,questions:items.map(x=>x.q),reviews:Object.values(S.reviews).filter(r=>ids.has(r.id)),attempts:items.flatMap(x=>x.atts),challenges:Object.values(S.challenges).filter(c=>ids.has(c.question_id))},null,2),"application/json");}
    ...
    download(base+".zip",new Blob([E.zipStore(files)],{type:"application/zip"}));}
  else if(ex.format==="print"){const html=printHtml(ex.title,items,opts,ex.sketches?await sketchMap(items,true):{});openPrint(html);}
  if(!copyOnly&&ex.format!=="print"){DLG.close();toast("Exported "+plural(items.length,"question")+".");}
}
...
function openPrint(html){
  ...
  else{download(E.safeName(S.ex.title)+".html",html,"text/html;charset=utf-8");DLG.close();toast("Pop-up blocked: saved as an HTML file instead. Open it and print.",5000);}
}
...
document.addEventListener("click",async e=>{
  ...
  switch(act){
    ...
    case "closeBackup":{const c=b.dataset.code;download(c+"-backup-"+S.today+".json",JSON.stringify(bundle(c),null,2),"application/json");S.closing[c].backedUp=true;render();break;}
...
async function boot(){
  ...
  const data=await AGStore.init();COLLS.forEach(c=>S[c]=data[c]||{});
  COLLS.forEach(c=>Object.entries(S[c]).forEach(([id,d])=>{if(d&&typeof d==="object")d.id=id;}));
  if(!Object.keys(S.questions).length&&!S.settings.seedMerged){await mergeSeed(true);S.settings.seedMerged=true;saveSettings();}
  S.ready=true;route();
}
boot();
})();
